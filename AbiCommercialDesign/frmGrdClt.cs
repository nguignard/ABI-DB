    ...
    public partial class frmGrdClt : Form
    {
        ...
        private void btnCltDspSupprimer_Click(object sender, EventArgs e)
        {
            ...
            if (rep == DialogResult.OK)
            {
                ...
                foreach (TClient tc in Donnees.Db.TClient.ToList()) // on recherche le Client correspondant dans la collection des cliens
                {
                    if (tc.IdClient == idClient)
                    {
                        tclient = tc;
                    }
                }
                Donnees.Db.TClient.Remove(tclient);//suppresssion de la liste des Clients
                Donnees.Db.TClient.SaveChanges();
        ...
        private void grdCltDsp_DoubleClick(object sender, EventArgs e)
        {
            ...
            foreach (TClient tc in Donnees.Db.TClient.ToList())
            {
                if (tc.IdClient == idClient)
                {
                    client = Donnees.convertToClient(tc); /////?????? Convertir les contacts
                }
            }
        }
